...
from .sources import fetch_sidof_notes
...
def _write_sidof_date(publication_date: date, root: Path, notes: list[dict[str, object]]) -> dict[str, object]:
    ...
    for edition in sorted(grouped):
        ...
        catalog = {
            ...
            "source": {
                ...
                "edition": edition,
                "index_url": f"https://sidof.segob.gob.mx/dof/sidof/notas/{publication_date:%d-%m-%Y}",
            },
    ...
    site_manifest = {
        ...
        "publication_date": date_key,
        "note_count": len(flattened),
        "insights": build_insights(flattened),
    }
...
def sync_date(
    ...
    result: MonitorResult = run_monitor(publication_date, root, fetch=dof_fetch)
    return {
